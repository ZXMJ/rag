...
def index_data(df_docs, source, index_name, index_name_knn):
    ...
    for index, row in df_docs.iterrows():
        ...
        for j, text in enumerate(passages):
            # Creating document for the regular index
            doc = {
                "query_id": query_id,
                "source": source,
                "text_field": text,
                "url": urls[j] if j < len(urls) else None  # Use URL if available
            }

            # Generating the embedding for the KNN index
            text_embedding = model.encode(text)

            # Creating document for the KNN index
            doc_knn = {
                "query_id": query_id,
                "source": source,
                "text": text,
                "text_embedding": text_embedding.tolist()  # Convert numpy array to list if needed
            }

            # Indexing the document to the regular index
            try:
                response = es_client.index(index=index_name, body=doc)
                print("Indexed to regular index:", response)
            except Exception as e:
                print(f"Error indexing document to regular index: {e}")

            # Indexing the document to the KNN index
            try:
                response = es_client.index(index=index_name_knn, body=doc_knn)
                print("Indexed to KNN index:", response)
            except Exception as e:
                print(f"Error indexing document to KNN index: {e}")
...
# df_corpus = pd.read_json("/root/.cache/huggingface/datasets/microsoft___ms_marco/v2.1/0.0.0/a47ee7aae8d7d466ba15f9f0bfac3b3681087b3a/ms_marco-validation.arrow")



# 加载 MS MARCO 数据集的 validation 分割
validation_dataset = load_dataset('microsoft/ms_marco', 'v2.1', split='validation')
