...
def processESIndex_RAG(df_squad, index):
    ...
    for ind in df_squad.index:
        ...
        # 提取 'answers', 'passages', 'query' 等字段
        answers = data['answers']
        passages = data['passages']
        query = data['query']
        # 统计每个数据项中的段落和问题数量
        content = passages['passage_text']
        squad_count += len(content)
        # 遍历每个段落
        for i in range(len(content)):
            story = content[i]  # 段落文本
            qas = data['passages']['is_selected']  # 选择状态 (假设是问题的索引列表)
            question_count += len(qas)
            print("Questions----", question_count)
            # 遍历每个问题
            for j in range(len(qas)):
                # 这里假设 'qas' 包含了问题文本和答案
                # 由于数据集中没有明确说明如何获取问题和答案，假设 'qas' 直接是问题的索引列表
                # 我们需要重新考虑如何提取问题和答案
                question = query  # 查询的文本
                gold_answer = answers[0]  # 取第一个答案
...
#     print("search_query_str:", search_query_str)
...
df_questions = validation_dataset.to_pandas()
